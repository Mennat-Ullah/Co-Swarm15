...
bool Frontier::isFrontier(Mnode* nd)
{
	...
	rep(n, 0, nd->num_nbrs)
	{
		...
		else
		{
			...
			{
				Frontiers[Frontiers_Num] = new Map_Node::Node();
...
void Frontier::Calculate_Frontiers(Uint x, Uint y)
{
	...
	vis.resize(map->get_rows());
	rep(i, 0, map->get_rows())
		vis[i].resize(map->get_cols(), false);
	Q.push(point(x, y));
	vis[x][y] = true;
	while (!Q.empty())
	{
...
Mnode*  Frontier::optimal_Frontier(Uint x, Uint y)
{
	...
	struct sort_pred
	{
		bool operator()(const std::pair<Map_Node::Node*, Uint> &left, const std::pair<Map_Node::Node*, Uint>&right)
		{
